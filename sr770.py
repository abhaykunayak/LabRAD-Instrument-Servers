...
class SR770Server(GPIBManagedServer):
    ...
    def identify_device(self, c, server, address):
        ...
        try:
            ...
            if resp == 'Stanford_Research_Systems,SR770,s/n24248,ver091':
    ...
    @setting(9, seconds= ['v{seconds of timeout}'], returns='')
    def set_timeout_gpib(self, c, seconds):
        """
        Experimental, set the gpib timeout for all of the queries of the GPIB interface with this instrument.
        Not sure if it works, (trusting the code written in GPIB.py for timeout)
        """
    ...
    def power_spectral_amplitude(self, c, trace):
        ...
        #Read from device
        yield dev.write('SPEB?%d\n' %trace)
        bytes = yield dev.read_raw()
    ...
    def power_spectral_amplitude_ASCII(self, c, trace):
        ...
        THIS FUNCTION DOES NOT WORK BECAUSE THE TIMEOUT SETTINGS ARE NOT LONG ENOUGH SET TIMEOUT TO 10s??
        """
        dev = self.selectedDevice(c)
        #Clear all status bytes
        yield dev.clearStatusBytes()
    ...
    @setting(104, trace='i{trace}', returns='i')
    def setup_for_read(self, c, trace):
        """
        Setup some of the settings of the sr770 for proper reading out.
        """
        dev = self.selectedDevice(c)
        #Clear all status bytes
        yield dev.clearStatusBytes()

        #check display and set
        yield self.display(c, trace, 0)
        disp = yield self.display(c, trace)
        if disp[1]!='LOG MAG':
            raise Exception('Setting display log mag failed')
        

    @setting(103, trace='i{trace}', returns='*2v{[freq,sqrt(psd)]}')
    def readpsdout(self, c, trace):
        """Get the trace in spectral amplitude (RMS) units, This is a faster version of power
        spectral amplitude without error checks for the averaging, used only really for dataline streaming
        to something else.
...
def unpackBinary(data):
    #print('unpackBinary in server, data length',len(data)) commented out for speed.
    if len(data)==800:
